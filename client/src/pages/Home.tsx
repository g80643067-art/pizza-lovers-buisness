...
export default function Home() {
  ...
  return (
    <div className="site-shell">
      ...
      <main>
        <section className="hero" id="home"><img className="hero-photo" src={HERO_IMAGE} alt="" /><div className="container hero-grid"><div className="hero-copy"><div className="hero-proof"><span className="eyebrow"><i /> Veg-only pizza at Takiya Mela Ground</span><span className="hero-veg-seal"><i /> 100% Veg only</span></div><h1 className="display hero-title">Pizza that<br /><em>pulls you closer.</em></h1><p className="hero-kicker"><b>Fresh. Hot. Loaded with Love.</b><br />Choose your pizza at the counter, then finish your order when you&apos;re ready.</p><div className="hero-actions"><a className="button-primary" href="#menu">Choose pizza <ArrowUpRight size={15} /></a><button type="button" className="button-quiet" onClick={openCart}>Your order <ShoppingBag size={15} /></button></div><div className="hero-meta"><div className="rating-chip"><Star size={16} /><span><strong>4.3 / 5</strong><small>58 Reviews</small></span></div><div className="hero-status"><b>Open until 10 PM</b>₹1–200 per person</div></div></div><div className="hero-stage"><Suspense fallback={null}><PizzaScene /></Suspense></div></div><div className="hero-rail">Scroll to savour</div></section>

        <div className="marquee" aria-hidden="true"><div className="marquee-track">{Array.from({ length: 2 }, (_, index) => <span key={index}>Veg-only pizza <i /> Fresh from the oven <i /> Pick your slice <i /> Takiya Patan <i /> Ready to order <i /></span>)}</div></div>
